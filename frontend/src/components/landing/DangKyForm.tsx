  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          apiError.response?.data?.error?.message || "Đã xảy ra lỗi. Vui lòng thử lại sau.",
        variant: "destructive",
      });
    } finally {
  ...
  if (isSuccess) {
    return (
      ...
      >
        ...
        <h3 className="text-2xl font-bold text-adk-green mb-2">Đăng ký thành công!</h3>
    ...
    >
      ...
      <div className="flex items-start space-x-2">
        <Checkbox
          ...
          onCheckedChange={(checked) => setValue("dongYDieuKhoan", checked === true)}
        ...
        <div className="grid gap-1.5 leading-none">
          ...
          {errors.dongYDieuKhoan && (
            <p className="text-sm text-red-500">{errors.dongYDieuKhoan.message}</p>
          )}
        </div>
      </div>

      {/* Submit Button */}
      <Button type="submit" variant="adk" size="xl" className="w-full" disabled={isSubmitting}>
        {isSubmitting ? (
          <>
